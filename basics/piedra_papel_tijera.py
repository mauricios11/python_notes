...
machine = "piedra"#random.choice(opciones)

print(f'\n elección de {user_name}: "{user}" elección de la computadora: "{machine}" \n')
if user in opciones:
    while user == machine:
        print("Empate! \n volvamos a jugar para el desempate")
        user = input("Elige una opción entre las tres posibles: ").lower()
    ...
    if user == "piedra":
        if machine == "tijera":
         print(f" {user} le gana a {machine} \n La computadora gana!")
        else:
         #papel
